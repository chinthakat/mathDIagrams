...
export default function Shape3D({ shapeProps, isSelected, onSelect, activeTool, onChange }) {
  ...
  return (
    <React.Fragment>
      <group 
        ...
        scale={[shapeProps.scaleX || 1, shapeProps.scaleY || 1, shapeProps.scaleZ || 1]}
      onClick={(e) => {
        e.stopPropagation();
        onSelect();
      }}
    >
      ...
      {isSelected && activeTool && activeTool !== 'select' && (
        <TransformControls
          object={meshRef}
          mode={activeTool}
          onMouseUp={(e) => {
            if (onChange && meshRef.current) {
